...
for (const file of htmlFiles) {
	...
	for (const m of html.matchAll(refRe)) {
		...
		// Skip external and non-http schemes.
		if (/^(https?:)?\/\//.test(href)) continue;
		if (/^(mailto:|tel:|data:|javascript:)/.test(href)) continue;

		// In-page anchors: validate against the ids this page actually emits.
		// Heading slugs are generated by Astro's slugger, and guessing them
		// wrongly is easy — a hand-written `#400-gen_invalid_input_schema--…`
		// missed the real id by one character and would have shipped silently.
		if (href.startsWith('#')) {
			const id = href.slice(1);
			if (id && id !== '_top' && !pageIds.has(id)) {
				console.error(`BROKEN (no such anchor) ${page}\n    -> ${href}`);
				broken++;
			}
			checked++;
			continue;
