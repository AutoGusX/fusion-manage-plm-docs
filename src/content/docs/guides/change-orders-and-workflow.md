...
Full endpoint reference lives in [Relationships and Affected Items](/api/v3/relationships-and-affected-items/) (Managed Items / `views/11`) and [Workflow](/api/v3/workflow/) (transitions) — this page walks through the common sequence.
...
```
or, if you already have an item and want the COs affecting it, use `views/2` on the item itself (item → CO direction — see [Relationships and Affected Items](/api/v3/relationships-and-affected-items/) for how this direction was confirmed):
```
...
```
Returns `affectedItems[]`, each pointing to a real item elsewhere in the tenant. This is the CO → item direction (the opposite of step 1's `views/2`) — see [Relationships and Affected Items](/api/v3/relationships-and-affected-items/) for the full confirmed-direction detail; don't mix the two up.
...
```
**Must be a fully-qualified absolute URL** — a relative path here returns `400 GEN_INVALID_INPUT_SCHEMA` (confirmed live; see [Relationships and Affected Items](/api/v3/relationships-and-affected-items/)).
...
Confirmed live (2026-07-13): a successful transition returns `303`, not `200`/`204`. Some transitions carry business-rule preconditions surfaced as a `400` with a plain-English message (e.g. "Change tasks must be defined in Tasks Planning tab first") — the CO's state is left unchanged when that happens, so it's safe to just try a different transition. If a transition needs a comment, the required field is `comment` (singular) — `comments`/`workflowComments` both fail. See [Workflow](/api/v3/workflow/) for the full detail.

Don't confuse this with a **lifecycle** transition (revision/release state, a separate v1 XML-bodied endpoint — see [Items](/api/v3/items/)). Workflow transitions move a CO through its approval steps; lifecycle transitions move an *item* between release states.
