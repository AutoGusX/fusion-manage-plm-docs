...
In practice, a 401 on an otherwise-working integration means the bearer token expired, not that the credentials are wrong. Refresh the token (see [Authentication](/concepts/authentication/)) and retry before treating a 401 as a hard failure.
